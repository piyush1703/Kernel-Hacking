...
fork(void)
{
  ...
  np->parent = curproc;
  *np->tf = *curproc->tf;
  np->tickets=curproc->tickets;  // child process get same tickets as parent process
...
scheduler(void)
{
  ...
  for(;;){
    ...
    // Loop over process table looking for process to run.---->(xv6 implementation)
//    acquire(&ptable.lock);
//    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
//      if(p->state != RUNNABLE)
//        continue;
    ...
    unsigned int ticketCounter = 0; // Count of how many tickets have been processed  to track winning process
